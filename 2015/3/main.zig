...
fn part1 (allocator: std.mem.Allocator) !void {
    var map = std.AutoHashMap(Coordinate, usize).init(allocator);
    defer map.deinit();

    var current_coord = Coordinate{ .x = 0, .y = 0 };
    for (input) |c| {
        var val = map.get(current_coord);
        if (val == null) val = 1;
        _ = try map.fetchPut(current_coord, val.? + 1);
        current_coord = switch (c){
            '^' => Coordinate{ .x = current_coord.x, .y = current_coord.y - 1 },
            '>' => Coordinate{ .x = current_coord.x + 1, .y = current_coord.y },
            '<' => Coordinate{ .x = current_coord.x - 1, .y = current_coord.y },
            'v' => Coordinate{ .x = current_coord.x, .y = current_coord.y + 1 },
            else => current_coord,
        };
    }
    ...
    var total_houses: usize = 0;

    while(iterator.next()) |_| {
        total_houses += 1;
    }

    std.debug.print("Total Houses: {}", .{total_houses});
}
...
fn part2 (allocator: std.mem.Allocator) !void {
    ...
    var santa_coord = Coordinate{ .x = 0, .y = 0 };
    var robo_coord = Coordinate{ .x = 0, .y = 0 };

    var char_count: usize = 0;

    for (input) |c| {
        const current_coord = if (char_count % 2 == 0) &santa_coord else &robo_coord;
        char_count += 1;
