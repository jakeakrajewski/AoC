...
pub fn main() !void {
    ...
    var a_button: Button = undefined;
    var b_button: Button = undefined;
    ...
    while (true) {
        ...
        if (next_line) |line| {
            ...
            if (std.mem.eql(u8, first, "Button")) {
                var next = split.next();
                if (next) |n| {
                    var button: *Button = undefined;
                    if (std.mem.eql(u8, n, "A:")) {
                        button = &a_button;
                    } else if (std.mem.eql(u8, n, "B:")) {
                        button = &b_button;
                    }
                    next = split.next();
                    if (next) |x| {
                        button.x = try std.fmt.parseInt(u8, x[2..4], 10);
                        next = split.next();
                        if (next) |y| {
                            button.y = try std.fmt.parseInt(u8, y[2..4], 10);
                        }
                    }
                }
            } else if (std.mem.eql(u8, first, "Prize:")) {
                const x_line = split.next().?;
                const x_target = try std.fmt.parseInt(usize, x_line[2 .. x_line.len - 1], 10);
                const y_target = try std.fmt.parseInt(usize, split.next().?[2..], 10);

                const min_cost = Search(.{ .a_button = a_button, .b_button = b_button }, x_target + 10000000000000, y_target + 10000000000000);

                sum += min_cost;
                found += 1;
                std.debug.print("Found: {}", .{found});
            }
        } else {
            break;
        }
...
const Button = struct { x: u8 = 0, y: u8 = 0 };

const Pair = struct { a_button: Button, b_button: Button };

inline fn Search(buttons: Pair, target_x: usize, target_y: usize) usize {
    var i: usize = 0;
    while (true) {
        const b_x: usize = buttons.b_button.x * i;
        const b_y: usize = buttons.b_button.y * i;

        var a_press: usize = 0;
        while (true) {
            const a_x: usize = buttons.a_button.x * a_press;
            const a_y: usize = buttons.a_button.y * a_press;

            if (b_x + a_x > target_x or a_y + b_y > target_y) {
                break;
            }
            if (b_x + a_x == target_x and a_y + b_y == target_y) {
                return i + (a_press * 3);
            }
            a_press += 1;
        }
